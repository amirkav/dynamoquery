...
from dynamoquery import json_tools
...
from dynamoquery.dynamoquery_types import (
...
from dynamoquery.expressions import ConditionExpression, ConditionExpressionType
from dynamoquery.lazy_logger import LazyLogger
from dynamoquery.sentinel import SentinelValue
from dynamoquery.utils import chunkify
...
class DynamoTable(Generic[_RecordType], LazyLogger, ABC):
    ...
    dynamoquery_class: Type[DynamoQuery] = DynamoQuery
    ...
    def max_batch_size(self) -> int:
        return self.dynamoquery_class.MAX_BATCH_SIZE
    ...
    def clear_table(
        ...
        for records_chunk in chunkify(records, self.max_batch_size):
            ...
            self.dynamoquery_class.build_batch_delete_item(logger=self._logger).table(
                table_keys=self.table_keys,
                table=self.table,
    ...
    def batch_get(self, data_table: DataTable[_RecordType], consistent_read: bool = False) -> DataTable[_RecordType]:
        ...
        results: DataTable[Any] = (
            self.dynamoquery_class.build_batch_get_item(consistent_read=consistent_read, logger=self._logger)
    ...
    def batch_delete(self, data_table: DataTable[_RecordType]) -> DataTable[_RecordType]:
        ...
        results: DataTable[Any] = (
            self.dynamoquery_class.build_batch_delete_item(logger=self._logger)
            .table(table_keys=self.table_keys, table=self.table)
    ...
    def batch_upsert(
        ...
        results: DataTable[Any] = (
            self.dynamoquery_class.build_batch_update_item(logger=self._logger)
            .table(
                table_keys=self.table_keys,
                table=self.table,
            )
    ...
    def get_record(self, record: _RecordType) -> Optional[_RecordType]:
        ...
        result: DataTable[Any] = (
            self.dynamoquery_class.build_get_item(logger=self._logger)
            .table(table_keys=self.table_keys, table=self.table)
    ...
    def upsert_record(
        ...
        result: DataTable[Any] = (
            self.dynamoquery_class.build_update_item(
    ...
    def delete_record(
        ...
        record = self.normalize_record(self._convert_record(record))
        result: DataTable[Any] = (
            self.dynamoquery_class.build_delete_item(
                condition_expression=condition_expression,
                logger=self._logger,
            )
    ...
    def scan(
        ...
        Arguments:
            ...
            limit -- Max number of results.
        """
        query = self.dynamoquery_class.build_scan(
            filter_expression=filter_expression,
            logger=self._logger,
        )
        if limit:
    ...
    def query(
        ...
        query = self.dynamoquery_class.build_query(
